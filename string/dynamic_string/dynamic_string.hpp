...
private:
  ...
  static int __lcp_naive(node *v, const dynamic_string &b, int l1, int l2, int &oksz, ull &ok){
    if(!v || l2 >= b.size()) return 0;
    if(l1 == 0 && l2 + v->sz <= b.size()){
    ...
    int szl = v->l ? v->l->sz : 0;
    if(szl + 1 <= l1) return __lcp_naive(v->r, b, l1 - (szl + 1), l2, oksz, ok);
    int left_cross = szl - l1, L = __lcp_naive(v->l, b, l1, l2, oksz, ok);
    if(L != left_cross) return L;
    l2 += left_cross;
    if(l2 >= b.size() || v->val != b.get(l2)) return L;
    ok = rh::add(ok, rh::mul(v->val, rh::rpow[oksz]));
    oksz++;
    return L + 1 + __lcp_naive(v->r, b, 0, l2 + 1, oksz, ok);
  }
  static std::vector<node*> bpath;
  
  static int __lcp(node *v, node *u, int l1, int l2){
    ...
    int szl = v->l ? v->l->sz : 0;
    if(szl + 1 <= l1) return __lcp(v->r, u, l1 - (szl + 1), l2);
    int left_cross = szl - l1, L = __lcp(v->l, u, l1, l2);
    if(L != left_cross) return L;
    l2 += left_cross;
    while(u){
      int szu = (u->l ? u->l->sz : 0) + 1;
    ...
    if(!u || l2 >= u->sz || v->val != get_inner(u, l2)) return L;
    return L + 1 + __lcp(v->r, u, 0, l2 + 1);
  }
public:
  dynamic_string(): root(nullptr){}
  template<typename T>
  dynamic_string(const std::vector<T> &v){
    if(v.empty()){
      root = nullptr;
      return;
    }
  ...
  static int lcp(const dynamic_string &a, const rolling_hash_string &b, int l1, int l2){
    return __lcp(a.root, b, l1, l2);
  }
  static int lcp(const rolling_hash_string &a, const dynamic_string &b, int l1, int l2){
    return lcp(b, a, l2, l1);
  }
  static int lcp(const dynamic_string &a, const dynamic_rolling_hash_string &b, int l1, int l2){
    int oksz = l2;
    ull ok = b.hash_range(l2);
    return __lcp(a.root, b, l1, l2, oksz, ok);
  }
  static int lcp(const dynamic_rolling_hash_string &a, const dynamic_string &b, int l1, int l2){
    return lcp(b, a, l2, l1);
  }
  static int lcp(const dynamic_string &a, const dynamic_string &b, int l1, int l2){
    return __lcp(a.root, b.root, l1, l2);
  }
  static int lcp_naive(const dynamic_string &a, const dynamic_string &b, int l1, int l2){
    int oksz = l2;
    ull ok = b.hash_range(0, l2);
    return __lcp_naive(a.root, b, l1, l2, oksz, ok);
  }
};
#endif