...
struct lazy_dynamic_binary_string{
  ...
  static int __lcp(node *v, lazy_dynamic_binary_string &b, int l1, int l2, int &oksz, ull &ok){
    ...
    if(l2 + mid_cross > b.size() || vx != b.hash_range(l2, l2 + mid_cross)){
      ...
      ull x = v->bit.x >> l1;
      ull y = lazy_dynamic_binary_string::__enumerate64(b.root, l2, l2 + len);
...
public:
  ...
  lazy_dynamic_binary_string(): root(nullptr){}
  lazy_dynamic_binary_string(const std::vector<bool> &bits){
    if(bits.empty()){
      root = nullptr;
      return;
    }
  ...
  static int lcp(lazy_dynamic_binary_string &a, lazy_dynamic_binary_string &b, int l1, int l2){
    int oksz = l2;
    ull ok = b.hash_range(l2);
    return __lcp(a.root, b, l1, l2, oksz, ok);
  }
};
std::vector<std::vector<unsigned long long>> lazy_dynamic_binary_string::htable;
lazy_dynamic_binary_string::node *lazy_dynamic_binary_string::tmp_node = nullptr;
#endif