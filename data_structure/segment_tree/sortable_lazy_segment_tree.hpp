#ifndef _SORTABLE_LAZY_SEGMENT_TREE_H_
#define _SORTABLE_LAZY_SEGMENT_TREE_H_
#include <vector>
#include <iostream>
#include <cassert>
#include <numeric>
...
struct sortable_lazy_segment_tree{
...
private:
  struct node{
    ...
    node(Key k, Val val, unsigned int p): k(k), kmin(k), kmax(k), val(val), sum(val), lazy(id_lazy()), flip(false),
  ...
  void propagate(node *v, Lazy x){
    ...
    v->sum = apply(v->sum, x, 0, v->sz_all);
  }
  void push_down(node *v){
    ...
    if(v->lazy != id_lazy()){
      if(v->inner_l) propagate(v->inner_l, v->lazy);
      if(v->inner_r) propagate(v->inner_r, v->lazy);
      if(v->outer_l) propagate(v->outer_l, v->lazy);
      if(v->outer_r) propagate(v->outer_r, v->lazy);
      v->lazy = id_lazy();
    }
  }
  ...
  void update_range_inner(node *v, int l, int r, Lazy x){
    if(!v) return;
    if(l == 0 && r == v->sz_all){
      propagate(v, x);
      return;
    ...
    if(l < szl){
      if(r <= szl){
        update_range_inner(v->inner_l, l, r, x);
        update(v);
        return;
      }
      update_range_inner(v->inner_l, l, szl, x);
      l = szl;
    }
    if(szr < r){
      if(szr <= l){
        update_range_inner(v->inner_r, l - szr, r - szr, x);
        update(v);
        return;
      }
      update_range_inner(v->inner_r, 0, r - szr, x);
      r = szr;
    }
    if(l != r) v->val = apply(v->val, x, 0, 1);
    update(v);
  }
  void update_range_outer(node *v, int l, int r, Lazy x){
    if(!v) return;
    if(l == 0 && r == v->sz_all){
      propagate(v, x);
      return;
    ...
    if(l < szl){
      if(r <= szl){
        update_range_outer(v->outer_l, l, r, x);
        update(v);
        return;
      }
      update_range_outer(v->outer_l, l, szl, x);
      l = szl;
    }
    if(szr < r){
      if(szr <= l){
        update_range_outer(v->outer_r, l - szr, r - szr, x);
        update(v);
        return;
      }
      update_range_outer(v->outer_r, 0, r - szr, x);
      r = szr;
    }
    if(l != r) v->val = apply(v->val, x, 0, 1);
    update(v);
  ...
  node *root;
public:
  sortable_lazy_segment_tree(): root(nullptr){}
  sortable_lazy_segment_tree(std::vector<std::pair<Key, Val>> &v): root(v.empty() ? nullptr : build(0, v.size(), v)){}
  ...
  void update(int l, int r, Lazy x){
    if(r == l) return;
    return update_range_outer(root, l, r, x);
  }
  // [l, r)の値
