#ifndef _BEATS_CHMIN_H_
#define _BEATS_CHMIN_H_
#include <vector>
#include <limits>
#include <algorithm>
#include <cassert>
...
private:
  ...
  struct T{
    ...
    int max_cnt;
    Val upper;
    T(): max(minf), second_max(minf), sum(0), max_cnt(0), upper(inf){}
  };
  std::vector<T> val;
...
public:
  beats_chmin(): M(0){}
  beats_chmin(int n, Val x): N(n), M(1 << ceil_pow2(n)), val(2 * M - 1){
    for(int i = 0; i < n; i++){
      val[M - 1 + i].max = val[M - 1 + i].sum = x, val[M - 1 + i].max_cnt = 1;
    }
    for(int i = M - 2; i >= 0; i--) merge_val(val[i], val[i * 2 + 1], val[i * 2 + 2]);
  }
  template<typename H>
  beats_chmin(const std::vector<H> &v): N(v.size()), M(1 << ceil_pow2(N)), val(2 * M - 1){
    for(int i = 0; i < v.size(); i++){
      val[M - 1 + i].max = val[M - 1 + i].sum = v[i], val[M - 1 + i].max_cnt = 1;
    }
    for(int i = M - 2; i >= 0; i--) merge_val(val[i], val[i * 2 + 1], val[i * 2 + 2]);
  }
